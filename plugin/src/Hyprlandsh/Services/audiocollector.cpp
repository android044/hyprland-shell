...
Q_LOGGING_CATEGORY(lcAc, "hyprlandsh.services.ac", QtInfoMsg)
Q_LOGGING_CATEGORY(lcAcWorker, "hyprlandsh.services.ac.worker", QtInfoMsg)

namespace hyprlandsh::services {
...
PipeWireWorker::PipeWireWorker(std::stop_token token, AudioCollector* collector)
    ...
    m_stream = pw_stream_new_simple(pw_main_loop_get_loop(m_loop), "hyprland-shell", props, &events, this);
    if (!m_stream) {
