...
class Tokens : public QQuickAttachedPropertyPropagator, public QQmlParserStatus {
    ...
    Q_PROPERTY(const hyprlandsh::config::AppearanceRounding* rounding READ rounding NOTIFY sourceChanged)
    Q_PROPERTY(const hyprlandsh::config::AppearanceSpacing* spacing READ spacing NOTIFY sourceChanged)
    Q_PROPERTY(const hyprlandsh::config::AppearancePadding* padding READ padding NOTIFY sourceChanged)
    Q_PROPERTY(const hyprlandsh::config::AppearanceFont* font READ font NOTIFY sourceChanged)
    Q_PROPERTY(const hyprlandsh::config::AppearanceTransparency* transparency READ transparency NOTIFY sourceChanged)
    Q_PROPERTY(const hyprlandsh::config::SizeTokens* sizes READ sizes NOTIFY sourceChanged)
    Q_PROPERTY(const hyprlandsh::config::AnimTokens* anim READ anim NOTIFY sourceChanged)
